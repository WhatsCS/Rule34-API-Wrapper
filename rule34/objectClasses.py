class Rule34Post:
    ...
    def parse(self, post):
        ...
        self.sample_height = (
            int(post["@sample_height"]) if "@sample_height" in post else None
        )
        self.sample_width = (
            int(post["@sample_width"]) if "@sample_width" in post else None
        )

        # PREVIEW VERSION - A TINY version of the image, suitable for thumbnails
        self.preview_url = str(post["@preview_url"]) if "@preview_url" in post else None
        self.preview_height = (
            int(post["@preview_height"]) if "@preview_height" in post else None
        )
        self.preview_width = (
            int(post["@preview_width"]) if "@preview_width" in post else None
        )
